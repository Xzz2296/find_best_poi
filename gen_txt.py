...
# 读取Excel文件
excel_file = r'E:\xpj\research\POI\desc_poi3.csv'  # 替换成你的Excel文件路径
...
# 指定保存文件的目录
output_directory = 'caption_prompt'  # 替换成你想要保存文件的目录路径

# 使用tqdm显示进度
for index, row in tqdm(df.iterrows(), total=total_rows, desc="生成.txt文件进度"):
    file_name = os.path.splitext(row['image'])[0] + '.txt'  # 去掉扩展名并添加.txt扩展名
